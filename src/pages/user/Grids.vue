/** 栅格组件 使用方式： 传入栅格数组，格式如下 grids: [ { icon: 'icon-plus',
name: '测试', path: { path: '/pages/client/home/main', isTab: true },openType:''
}, ] */
<template>
  <div class="grids">
  ...
  computed: {
    width() {
      if (this.grids.length > this.limit) {
        return 200 / this.limit
      }
      return 200 / this.grids.length
    }
  },
  methods: {
    onClick(path) {
